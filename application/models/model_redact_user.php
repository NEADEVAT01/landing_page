...
class Model_Redact_User extends Model
{
    ...
    {
        $res = mysqli_query($this->link, "DELETE FROM `$name_table` WHERE user_id = $elem_id");
        if (!$res) {
            die(mysqli_error($this->link));
        }
    }
    ...
    {
        $res = mysqli_query($this->link, "UPDATE `$name_table` SET $update_params WHERE user_id = $elem_id");
        if (!$res) {
            die(mysqli_error($this->link));
        }
    }
